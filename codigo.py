...
def main():
    ...
    # Título do app
    st.title("🌐 Tradutor com Mistral AI")
    ...
    with col1:
        ...
        texto_original = st.text_area("Digite o texto para traduzir", height=240)
        ...
        with st.container():
            ...
            if botao_traduzir and texto_original and chave_api:
                with st.spinner(f"Traduzindo para {idioma_destino}..."):
                    ...
                    if texto_traduzido:
                        area_traducao.text_area("Texto traduzido", value=texto_traduzido, height=240, disabled=True)
                        st.success("Tradução concluída!")
                    else:
                        area_traducao.empty()
                        st.error(f"Erro ao traduzir: {erro}")
            else:
                area_traducao.text_area("Texto traduzido", value="", height=240, disabled=True)
